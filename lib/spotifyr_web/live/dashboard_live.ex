defmodule SpotifyrWeb.DashboardLive do
  ...
  import SpotifyrWeb.SpotifyComponents

  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> assign_async(:me, fn ->
       result_to_async_result(Spotifyr.Spotify.get_profile(socket.assigns.current_user), :me)
     end)
  ...
  def render(assigns) do
    ...
    <.simple_form for={@track_form} phx-submit="save">
      <SpotifyrWeb.CoreComponents.input label="Track URL" field={@track_form[:url]} />
      <:actions>
  ...
  def handle_event("save", %{"url" => url}, socket) do
    ...
    {:ok, audio_features} =
      Spotifyr.Spotify.get_audio_features(socket.assigns.current_user, track_id)
